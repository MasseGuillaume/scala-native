...
object Asserts {
    ...
    expr.tree match {
      case Literal(Constant(s: String)) =>
        try {
          ...
          val compileTimeOnlyType =
            typeOf[scala.reflect.internal.annotations.compileTimeOnly]

          val compileTimeOnlyTree = tree2.collect {
            case t =>
              Option(t.symbol)
                .map(_.annotations)
                .toSeq
          ...
          case TypecheckException(pos, msg) =>
            c.Expr[CompileError](
              q"""utest.CompileError.Type(${calcPosMsg(pos)}, $msg)""")
          case ParseException(pos, msg) =>
            c.Expr[CompileError](
              q"""utest.CompileError.Parse(${calcPosMsg(pos)}, $msg)""")
          case e: Exception =>
  ...
  def interceptProxy[T: c.WeakTypeTag](c: Context)(exprs: c.Expr[Unit])(
      t: c.Expr[ClassTag[T]]): c.Expr[T] = {
    import c.universe._
    val typeTree = implicitly[c.WeakTypeTag[T]]

    val x =
      Tracer[Unit](c)(q"utest.asserts.Asserts.interceptImpl[$typeTree]", exprs)
    c.Expr[T](q"$x($t)")
  }
  ...
  def assertMatchProxy(c: Context)(t: c.Expr[Any])(
...
trait Asserts[V[_]] {
  ...
  def assertMatch(t: Any)(pf: PartialFunction[Any, Unit]): Unit =
    macro Asserts.assertMatchProxy
