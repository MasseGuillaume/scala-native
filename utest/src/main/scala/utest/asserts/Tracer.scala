 ...
 */
object Tracer {
  def wrapWithLoggedValue(
      c: Context)(tree: c.Tree, loggerName: c.TermName, tpe: c.Type) = {
    import c.universe._
  ...
  def apply[T](c: Context)(func: c.Tree, exprs: c.Expr[T]*): c.Expr[Unit] = {
          ...
          case i: Typed =>
            i.tpe match {
              case t: AnnotatedType
                  ...
                  if t.annotations.map(_.tpe) == Seq(
                    typeOf[utest.asserts.Show]) =>
    ...
    val trees = exprs.map(expr =>
      q"${expr.tree.pos.lineContent.trim} -> (($loggerName: ${tq""}) => ${tracingTransformer
        .transform(expr.tree)})")
