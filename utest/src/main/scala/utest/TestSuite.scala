...
abstract class TestSuite
  ...
  def assertPrettyPrint[T: DummyTypeclass](t: T) =
    t.toString.take(utestTruncateLength)
...
trait TestSuiteMacro {
   ...
   */
  def apply(expr: Unit): framework.Tree[framework.Test] =
    macro framework.TreeBuilder.applyImpl
}
object TestSuite extends TestSuiteMacro {

  trait Retries extends utest.TestSuite {
    val utestRetryCount: Int
    override def utestWrap(body: => Future[Any])(
