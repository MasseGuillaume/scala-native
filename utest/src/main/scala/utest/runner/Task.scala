...
class Task(val taskDef: TaskDef,
           args: Array[String],
           path: String,
           runUTestTask: (Seq[String], Seq[Logger], String,
                          EventHandler) => Future[Unit])
    extends sbt.testing.Task {

  def tags(): Array[String] = Array()

  def execute(eventHandler: EventHandler,
              loggers: Array[Logger]): Array[testing.Task] = {
  ...
  private def executeInternal(eventHandler: EventHandler,
                              loggers: Array[Logger]) = {
