...
case class SkippedOuterFailure(errorPath: Seq[String], outerError: Throwable)
    extends Exception(errorPath.mkString("."), outerError)

/**
 * Indicates that there was no test to run at the path you provided
 */
case class NoSuchTestException(path: String*)
    extends Exception("[" + path.mkString(".") + "]")
...
case class MultipleErrors(thrown: Throwable*)
    extends Exception(thrown(0).getMessage, thrown(0).getCause)
