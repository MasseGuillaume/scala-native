...
class TestTreeSeq(tests: Tree[Test]) {
  ...
  def runFuture(onComplete: (Seq[String], Result) => Unit,
                ...
                outerError: Future[Option[SkippedOuterFailure]] =
                  Future.successful(Option.empty[SkippedOuterFailure]))(
    ...
    Future {
      ...
      def runChildren(tail: Seq[Tree[Test]],
                      results: List[Tree[Result]],
                      index: Int): Future[List[Tree[Result]]] = {
      ...
      futurized.map {
        ...
        futureResults.map { results =>
          ...
          val result = Result(tests.value.name,
                              res1,
                              end.time.toMillis - start.time.toMillis)
